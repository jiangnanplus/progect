...
class CodStatistics extends Model
{
    //指定数据库
    protected $connection = 'mysql';
    //指定表名
    protected $table ="experss_cod_statistics";
    //指定id
     ...
     * $task_id 任务id
     */
    public function create_cod_statistic($coddata,$task_id)
    {
        ...
        //统计每年的总数量
        foreach($this->array_counts($coddata,"year") as $k=>$v){
            $data_by_year = [
                'time_by' => "year",
                'quantity' => $v['rows'],
                'dimension_site' => "no",
        ...
        //统计每月的总数量
        foreach($this->array_counts($coddata,"month") as $k=>$v){
            $data_by_month = [
                'time_by' => "month",
                'quantity' => $v['rows'],
                'dimension_site' => "no",
        ...
        //统计每周的总数量
        foreach($this->array_counts($coddata,"week") as $k=>$v){
            $data_by_week = [
                'time_by' => "week",
                'quantity' => $v['rows'],
                'dimension_site' => "no",
        ...
        //统计每日的总数量
        foreach($this->array_counts($coddata,"day") as $k=>$v){
            $data_by_day = [
                'time_by' => "day",
                'quantity' => $v['rows'],
                'dimension_site' => "no",
     ...
     * $order_by 排序方式
     */
    public function get_express_cod_data_by_rank($rank_time,$time_by,$dimensions,$type,$order_by)
    {
        //判断时间
